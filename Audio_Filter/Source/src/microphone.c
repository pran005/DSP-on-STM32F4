/** @Note : The following application is written on the basis of ST's Application Note : AN3998 for Audio on STM32F4 Discovery Board **/ 
...
#define INTERNAL_BUFF_SIZE      64        // PDM buffer input size using formula given in AN3998
#define PCM_OUT_SIZE            16        // PCM buffer output size using formula given in AN3998

static uint32_t AudioRecInited = 0;       // stores the state of the audio recorder interface intialization
...
uint16_t RecBuf[PCM_OUT_SIZE];            // Main buffer pointer for the recorded data storing
uint32_t AudioRecChnlNbr = 1;             // Audio recording number of channels 1(mono)  Mic has only one channel as LR pin of Mic is Grounded on Discovery board
uint16_t* pAudioRecBuf;                   // A pointer to an buffer
uint32_t AudioRecCurrSize = 0;            // Current size of the recorded buffer

static uint16_t InternalBuffer[INTERNAL_BUFF_SIZE]; // Temp data sample storage
...
{ 
  ...
  {
    /* Enable CRC module as mentioned in AN3998 for functioning of PDMlib */
...
/* Check if the interface has already been initialized */
  if (AudioRecInited)
  {
    ...
    /* The Data transfer is performed in the SPI2_IRQHandler */
    /* Enable the SPI_I2S peripheral */
    I2S_Cmd(SPI2, ENABLE); 
...
{  
  ...
  if (SPI_GetITStatus(SPI2, SPI_I2S_IT_RXNE) != RESET)
  {
    ...
    InternalBuffer[InternalBufferSize++] = HTONS(app);    		// Data in Little-Endian for eg. 5000 = 0x1389 , but in l'il endian 0x89 0x13 so to retrieve correctly
																														 //  we exchange these bytes using HTONS in the PDMlib header file. 
    ...
    if (InternalBufferSize >= INTERNAL_BUFF_SIZE)
    {
      ...
      PDM_Filter_64_LSB((uint8_t *)InternalBuffer, (uint16_t *)pAudioRecBuf, volume , (PDMFilter_InitStruct *)&Filter);
			// call the callback function in headphone.c
